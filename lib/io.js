...
function IO() {
    ...
    this.setLogdir = function(logdir) {
        ...
        Object.keys(spec).forEach(function(name) {
            if (spec[name].type === "regulator") {
                ...
                this.timers[name].on("expired", function(name) {
                    if (this.logdir) {
                        fs.append(path.join(this.logdir, name + ".log"), new Date().getTime() + ":expired\n")
                    }
                ...
                if (!this.output[spec[name].serial]) {
                    ...
                    this.output[spec[name].serial].on("pin", function(pin, state) {
                        ...
                        if (pinName) {
                            if (this.logdir) {
                                fs.append(path.join(this.logdir, pinName + ".log"), new Date().getTime() + ":" + state + "\n")
                            }
                ...
                if (!this.temperature[spec[name].serial]) {
                    ...
                    this.temperature[spec[name].serial].on("temperature", function(input, value) {
                        ...
                        if (inputName) {
                            if (this.logdir) {
//                                 this.temperatureLogCount[inputName] = this.temperatureLogCount[inputName] || 0;

//                                 if (this.temperatureLogCount[inputName] === 0) {
                                    fs.append(path.join(this.logdir, inputName + ".log"), new Date().getTime() + ":" + value + "\n")
//                                 }

//                                 this.temperatureLogCount[inputName]++;
//
//                                 if (this.temperatureLogCount[inputName] >= 10) {
//                                     this.temperatureLogCount[inputName] = 0;
//                                 }
                            }
    ...
    this.setTimer = function(name, timeout, force) {
        if (this.logdir) {
            fs.append(path.join(this.logdir, name + ".log"), new Date().getTime() + ":" + timeout + "\n")
        }
    ...
    this.startRegulator = function(name, temperature, input, output) {
        if (this.logdir) {
            fs.append(path.join(this.logdir, name + ".log"), new Date().getTime() + ":" + temperature + ":" + input + ":" + output + "\n")
        }
    ...
    this.stopRegulator = function(name) {
        if (this.logdir) {
            fs.append(path.join(this.logdir, name + ".log"), new Date().getTime() + ":stopped\n")
        }
