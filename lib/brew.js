...
function Brew() {
    ...
    this.setup = function(io) {
        ...
        this.io.on("timerExpired", function() {
            this.checkCriteria();
        }.bind(this));
    ...
    this.execute = function() {
        ...
        for (var n = 0; n < step.actions.length; n++) {
            if (step.actions[n].type === "SetPins") {
                promises.push(this.io.setPins(step.actions[n].outputs));
    ...
    this.checkCriteria = function() {
        ...
        for (var n = 0; n < step.criteria.length; n++) {
            if (step.criteria[n].type === "AwaitTemperature") {
                //pass = pass && this.io.getTemperature(step.criteria[n].input) >== step.criteria[n].temperature;
            } else if (step.criteria[n].type === "AwaitTimer") {
                var hasExpired = this.io.hasTimerExpired(step.criteria[n].timer);

                if (hasExpired) {
                    step.timerExpired = step.timerExpired || {};
                    step.timerExpired[step.criteria[n].timer] = true;
                }

                pass = pass && hasExpired;
            } else if (step.criteria[n].type === "AwaitUser") {
                pass = pass && step.userConfirmed;
            }
        }
